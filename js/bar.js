class ProgressBar {
    ...
    static updateTimeLeft(bar, millisLeft) {
        ...
        // Change color to orange if less than 5 minutes
        if (millisLeft < 5 * 60 * 1000) {
            bar.timeLeftStatus.style.color = 'orange';
        } else {
            bar.timeLeftStatus.style.color = 'white'; // Default color
        }
    }
...
setInterval(getSchedules, 1000 * 60 * 60 * 24); // Check every 24 hours
reloadPage(7); // Reload the page at 7 AM
